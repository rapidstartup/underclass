...
export function Choice({ prompt, optionA, optionB, onChoice, disabled }: ChoiceProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
        ...
        >
          ...
          <span className="text-[15px] text-white/85 font-medium leading-snug relative z-10">{optionA}</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[15px] text-white/85 font-medium leading-snug relative z-10">{optionB}</span>
        </button>
      </div>
    </motion.div>
  );
